...
function chooseHole(holes) {
  // TODO: Write your code here.
  const ranNumber = randomInteger(0, 8);
  const hole = holes[ranNumber];
  if(hole === lastHole) {
...
function showUp() {
  ...
  const hole = chooseHole(document.querySelectorAll('.hole'));  // TODO: Update so that it use chooseHole()
...
function setDifficulty(event) {
  if(event.classList.contains("easy")) {
    difficulty = "easy";
  } else if(event.classList.contains("medium")) {
    difficulty = "medium";
  } else if(event.classList.contains("hard")) {
    difficulty = "hard";
  }
}
...
medButton.addEventListener("click", setDifficulty);
